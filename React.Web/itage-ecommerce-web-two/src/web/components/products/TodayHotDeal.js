	...
	useEffect(() => {
		...
		const dataOperationInUseEffect = async () => {
			...
			const response = await MakeApiCallAsync(
				Config.END_POINT_NAMES["GET_LATEST_HOT_PRODUCT"],
				null,
				param,
				headers,
				"POST",
			...
			if (response != null && response.data != null) {
				...
				setHopProduct(
					hProduct != null && hProduct != undefined ? hProduct[0] : {}
				);
			}

			const responseCustomerFav = await MakeApiCallAsync(
				Config.END_POINT_NAMES["GET_CUSTOMER_FAVOURITE_PRODUCTS"],
				null,
				param,
				headers,
				"POST",
			...
			if (
				responseCustomerFav != null &&
				responseCustomerFav.data != null
			) {
									...
									<Row className="row hot-deal-subcontain">
										<Col lg="4" md="4">
											<div className="hotdeal-right-slick border-0">
												...
												>
													{HotProduct &&
														HotProduct.ProductImagesJson?.map(
															(img, i) => {
																return (
																	<div
																		key={i}
										...
										<Col lg="6" md="6">
											{HotProduct != undefined &&
											HotProduct != null ? (
												<div
													className="hot-deal-center"
													style={{ display: "block" }}
												>
													<div>
														<div>
					...
					<Col lg="3">
						<div>
							<div className="media-banner border-0">
								...
								{CustomerFavouritProd &&
									CustomerFavouritProd.map((item, i) => (
										<div
											key={i}
