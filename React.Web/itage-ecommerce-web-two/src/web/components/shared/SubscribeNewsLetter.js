	...
	const submitSubscribeForm = async (event) => {
		...
		if (isValid) {
			...
			const response = await MakeApiCallAsync(
				Config.END_POINT_NAMES["INSERT_SUBSCRIBER"],
				null,
				param,
				headers,
				"POST",
			...
			if (response != null && response.data != null) {
				...
				if (detail[0].ResponseMsg == "Saved Successfully") {
					showSuccessMsg(
						"You have successfully subscribed to news channel!"
					);
		...
		const dataOperationFunc = async () => {
			...
			let arryRespLocalization =
				await GetLocalizationControlsJsonDataForScreen(
					GlobalEnums.Entities["SubscribeNewsLetter"],
					null
				);
			if (
								...
								>
									<div className="input-group">
										...
										<div className="input-group-prepend">
											<button
												type="submit"
												className="subscribe-bnt input-group-text telly"
											>
