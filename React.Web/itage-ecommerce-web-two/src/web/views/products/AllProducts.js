...
const AllProducts = () => {
	...
	const [SearchTerm, setSearchTerm] = useState(
		new URLSearchParams(search).get("SearchTerm")
	);
	...
	const setFilterValueInParent = async (e, value, type) => {
		...
		let brandsIdsCommaSeperated =
			ManufacturerID.length > 0 ? ManufacturerID.join(",") : "";
		...
		await getAllProductsAfterAnyFilterChange(
			1,
			categoriesIdsCommaSeperated,
			brandsIdsCommaSeperated,
	...
	const getAllProductsAfterAnyFilterChange = async (
		pageNumber,
		_categoryId,
		_manufacturerId,
		_sizeId,
		...
		try {
			...
			let responseAllProducts = await MakeApiCallAsync(
			...
			if (
				responseAllProducts != null &&
				responseAllProducts.data != null
			) {
				await setProductsList(
					JSON.parse(responseAllProducts.data.data)
				);
				let AllProducts = JSON.parse(responseAllProducts.data.data);
				await setTotalRecords(
					parseInt(AllProducts[0]?.TotalRecords ?? 0)
				);
							...
							>
								<div className="sticky-sidebar">
									<SiteLeftSidebarFilter
										setFilterValueInParent={
											setFilterValueInParent
										}
										setLeftSidebarOpenCloseFromFilter={
											setLeftSidebarOpenCloseFromFilter
										}
									/>
