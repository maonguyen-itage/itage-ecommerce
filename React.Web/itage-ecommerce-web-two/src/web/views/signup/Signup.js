			...
			if (isValid) {
				...
				if (response != null && response.data != null) {
					...
					if (
						...
						const responseLogin = await MakeApiCallAsync(
						...
						if (
							...
							if (
								loginData.length > 0 &&
								loginData[0].ResponseMsg != undefined &&
								loginData[0].ResponseMsg == "Login Successfully"
							) {
								//--save user data in session
								localStorage.setItem(
									"user",
									JSON.stringify(loginData[0])
								);
								dispatch(
							...
							navigate(
								"/" + getLanguageCodeFromSession() + "/login"
							);
						}
							...
							<div className="theme-card">
								...
								>
									<div className="form-row row">
										...
										<Col md="12">
											<button
												className="btn btn-sm btn-normal mb-3"
												type="submit"
										...
										<Col md="12">
											<p>
												{t("have_you_already_account")}{" "}
												<a
													to={`/${getLanguageCodeFromSession()}/login`}
													className="txt-default"
												></a>{" "}
												&nbsp;
												<Link
													to={`/${getLanguageCodeFromSession()}/login`}
													className="txt-default"
													id="lbl_singup_loginacnt"
												>
													{t("login")}
													{/* {LocalizationLabelsArray.length >
													0
